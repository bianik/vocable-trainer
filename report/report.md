...
#### Database password
In this [line](../myProjects.vocableTrainer/src/main/java/myProjects/vocableTrainer/app/VocableTrainerApp.java#L56) SonarCloud suggested to protect the database with a password. Since the database is running on an external server in a Docker container, which will be build and executed on every build (on Travis CI), the password would be needed to be set up on every build. I tried to do this in this Dockerfile, by starting the server during the build of the container, trying to execute the SQL command on the database to change a user's password and then stopping the server again. Unfortunately this does not work because calling the localhost inside a Docker container seems to be not that easy.  
