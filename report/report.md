...
I think that this problem is quite a interesting one so I wrote a bit more about the way I found the problem and also it's solution. (The links direct to the appropriate commit.)
...
The solution I will go for is the one to create a new method in the class Vocable [`Vocable compareTo(Vocable other)`](https://github.com/bianik/vocable-trainer/blob/1bfdd00834b8ca42e78be30513d4ef6cd74172ec/myProjects.vocableTrainer/src/main/java/myProjects/vocableTrainer/model/Vocable.java#L64).
...
#### Database password
In this line SonarCloud suggested to protect the database with a password. Since the database is running on an external server in a Docker container, which will be build and executed on every build (on Travis CI), the password would be needed to be set up on every build. I tried to do this in this Dockerfile, by starting the server during the build of the container, trying to execute the SQL command on the database to change a user's password and then stopping the server again. Unfortunately this does not work because calling the localhost inside a Docker container seems to be not that easy.  
